...
# play with the jump settings for height and distance
...
class Player(turtle.Turtle):
    ...
    def animate(self):
        ...
        if self.dx<0:
            if self.s == 'stick-L1.gif' and counter%10 == 0: # Every 10th time in the loop (timing of change)
                self.s = 'stick-L2.gif'
                self.shape(self.s)
...
class Game():
    def __init__(self):
        ...
        # Place background gif as tiles - 5 in a row in x and y (25x)
        ...
        # Place the platforms    
        self.platforms = []
        # y, x, gif (from bottom to top - the only reason I put y first)
        self.pf = [
    ...
    def update(self):
        ...
        # After pressing spacebar - jump(not the best solution!?)
        if self.player.jump == 'up':
            self.player.dy = 5 # Play around with this value for height

            # Play around with the dx*6 value for distance:
            self.player.goto(self.player.xcor()+self.player.dx*6, self.player.ycor()+self.player.dy)
            self.counter += 1
            if self.counter == 10:
            ...
            self.player.goto(self.player.xcor()+self.player.dx*6, self.player.ycor()+self.player.dy)
            #if self.player.ycor() <= self.player.current_y:
            if self.player.ycor() <= -220:
...
# Functions to assess basic collisions (same as for paddle check in pong):
