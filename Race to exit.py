...
# Place background gif (100x100 pixels as tiles - 5 in a row in x and y (25x)
...
# Place the platforms    
platforms = []
# y, x, gif (from bottom to top - this is the only reason I put y first)
pf = [
...
for i in pf:
    ...
    platform.shape(i[2])
    if i[2] == 'platform1.gif': # 100x10 pixels
        platform.width = 50 # distance from middle of platform or xcor()
        platform.height = 10
    elif i[2] == 'platform2.gif': # 66x10 pixels
        platform.width = 33
        platform.height = 10
    elif i[2] == 'platform3.gif': # 32x10 pixels
        platform.width = 16
        platform.height = 10
...
class Player(turtle.Turtle):
    ...
    def animate(self):
        if player.dx < 0:
            if player.s == 'stick-L1.gif' and counter%10 == 0: #  Every 10th time in the loop
                player.s = 'stick-L2.gif'
                player.shape(player.s)
            elif player.s == 'stick-L2.gif' and counter%10 == 0:
                player.s = 'stick-L3.gif'
                player.shape(player.s)
            elif player.s == 'stick-L3.gif' and counter%10 == 0:
                player.s = 'stick-L1.gif'
                player.shape(player.s)

        elif player.dx > 0:
            if player.s == 'stick-R1.gif' and counter%10 == 0:
                player.s = 'stick-R2.gif'
                player.shape(player.s)
            elif player.s == 'stick-R2.gif' and counter%10 == 0:
                player.s = 'stick-R3.gif'
                player.shape(player.s)
            elif player.s == 'stick-R3.gif' and counter%10 == 0:
                player.s = 'stick-R1.gif'
                player.shape(player.s)
